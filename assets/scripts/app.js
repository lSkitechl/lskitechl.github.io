(() => {
      ...
      const FLASH_GAP = 60;
      const MAIN_PAGE_REVEAL_DELAY = BOOT_SCREEN_FADE_TIME + FLASH_GAP + 520 + 60;

      const diagnosticSteps = [
            { value: "OK", trigger: 8 },
            { value: "OK", trigger: 20 },
            { value: "OK", trigger: 34 },
            { value: "028",trigger: 48 },
            {value: "UNKNOWN",trigger: 61 }
      ];
      ...
      function resetDiagnostics() {
        diagnosticRows.forEach(
          (row, index) => {
            const value = row.querySelector(".diagnostic-value");
            row.classList.remove("active", "flash");
      ...
      function updateDiagnostics(progress) {
        diagnosticRows.forEach((row, index) => {
            ...
            if (progress >= step.trigger && row.dataset.done !== "true") {
                row.dataset.done = "true";
                row.classList.add("active", "flash");

                const value = row.querySelector(".diagnostic-value");
                value.textContent = step.value;
                window.setTimeout(() => { row.classList.remove("flash"); }, 380);
            }
          }
        );
      }
      ...
      function updateStatus(progress) {
        const current = statusStages.find((stage) => progress <= stage.max);
        if (current) { statusLine.textContent =current.text; }
      }

      function calculateProgress(elapsed) {
        const normalized = Math.min(elapsed / BOOT_TIME, 1);
      ...
      function renderProgress(value) {
        ...
        const secondsLeft =Math.max(0, Math.ceil(BOOT_TIME * (1 - value / 100) / 1000));
        estimatedTime.textContent = `EST. 00:${String(secondsLeft).padStart(2, "0")}`;
        updateDiagnostics(rounded);
        updateStatus(rounded);
        if (rounded >= 63) { runtimeText.classList.add("active"); }
      }

      /* = BOOT LOOP = */
      function bootLoop(timestamp) {
        const elapsed = timestamp - bootStart;
        const progress = calculateProgress(elapsed);

        if (!bootStart) { bootStart = timestamp; }
        renderProgress(progress);
        if (progress >= 100) { finishBoot(); return; }
        animationFrame = requestAnimationFrame(bootLoop);
      }
      ...
      function finishBoot() {
        ...
        statusLine.textContent = "SYSTEM READY.";
        estimatedTime.textContent = "READY";
        runtimeText.textContent = "SYSTEM RUNTIME INITIALIZED.";
        bootFooter.textContent = "// SKITECH OS READY. ENTERING CORE. //";
        diagnosticRows.forEach((row) => { row.classList.add("active"); });
        bootScreen.classList.add("boot-complete");
        window.setTimeout(() => { screenFlash.classList.add("flash");}, BOOT_SCREEN_FADE_TIME + FLASH_GAP);
        window.setTimeout(() => { mainPage.classList.add("visible"); document.body.style.overflow = "auto";}, MAIN_PAGE_REVEAL_DELAY);
      }
      ...
      function startBoot() {
        ...
        runtimeText.classList.remove("active");
        runtimeText.textContent ="INITIALIZING SYSTEM RUNTIME...";
        bootFooter.textContent ="// STAND BY. INITIALIZING INTERFACE. //";
        percentage.textContent = "0";
        progressCount.textContent = "0";
        progressBar.style.width = "0%";
        statusLine.textContent ="BOOT SECTOR INITIALIZATION...";
        estimatedTime.textContent ="EST. 00:04";
        animationFrame =requestAnimationFrame(bootLoop);
      }
    
      replayButton.addEventListener("click", () => { startBoot(); } );
      /* = INITIALIZATION = */
      startBoot();
})();
